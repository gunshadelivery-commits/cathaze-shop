...
function toggleProductModal(show, mode = 'add') {
    ...
    if(!show) { document.getElementById('productForm').reset(); document.getElementById('variantContainer').innerHTML = ""; addVariant(); }
...
async function deleteFullProduct(name) {
    ...
    const variants = rawProducts.filter(p => p.name === name);
    for(let v of variants) { await fetch(GAS_URL, { method: 'POST', mode: 'no-cors', body: JSON.stringify({ action: "deleteProduct", name: v.name, size: v.size }) }); }
